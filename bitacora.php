...
$sql_base = "
    SELECT
        ...
        (COALESCE(i.costo_mano_obra,0) + COALESCE(i.costo_materiales_proveedor,0)
         ...
         + (COALESCE(i.horas_trabajadas,0) * COALESCE(i.tarifa_hora_aplicada,0))
         + (SELECT COALESCE(SUM(ir.costo_total),0) FROM incidencia_refacciones ir WHERE ir.incidencia_id = i.id)) AS costo_total,
        (COALESCE(i.costo_mano_obra,0) + COALESCE(i.costo_materiales_proveedor,0)
         + COALESCE(i.costo_materiales_comprados,0)
         + (SELECT COALESCE(SUM(ir.costo_total),0) FROM incidencia_refacciones ir WHERE ir.incidencia_id = i.id)) AS costo_total_visible
    FROM incidencias i
...
<div class="space-y-4 animate-fade-in"
    ...
    <div class="flex flex-col <?= usuario_prefiere_radio_sucursal() ? 'xl:flex-row xl:items-start' : 'lg:flex-row lg:items-center' ?> lg:justify-between gap-3">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <!-- Selector rápido de sucursal (dropdown — usuarios sin preferencia radio) -->
            <?php if ($ver_todas && !usuario_prefiere_radio_sucursal()): ?>
            <form method="GET" class="relative">
                <?php
                foreach ($_GET as $k => $v) {
