...
$default = [
    ...
    'horas_trabajadas' => '',
    // Materiales del trabajo externo: monto total y cómo se reparte.
    // 'materiales_origen': proveedor | interno | mixto (campos auxiliares, no son columnas)
    'costo_materiales_monto' => '',
    'materiales_origen' => 'proveedor',
    'materiales_pct_proveedor' => 100,
];
    ...
    if (!csrf_valido(input('_csrf'))) {
        ...
        if (empty($errores)) {
            try {
                ...
                // Admin puede ingresar fecha_resolucion manualmente; si no, auto-set al completar
...
<div class="max-w-5xl mx-auto animate-fade-in"
    ...
    <form method="POST" enctype="multipart/form-data" class="space-y-5" x-ref="formulario">
        ...
        <div class="bg-white rounded-xl border border-zinc-200 shadow-sm p-6">
            ...
            <p class="text-xs text-zinc-500 mb-4">Sin límite de archivos · hasta 30 MB cada uno. Formatos: imágenes, PDF, Word, Excel, ZIP, TXT.</p>

            <input type="file" name="adjuntos[]" multiple
                   x-ref="inputFiles" @change="agregarArchivos($event.target.files)"
                   class="hidden">
